...
module.exports = function(set){
	...
	var muinfo=set.muinfo,
		...
		alb_des = album_des(pinfo),
		str_des = music_des(muinfo),
		site_url = filterURL(infosite, pinfo?regx_official:regx_fyi, false),
	...
	return `${str_hashtag}${alb_des}${str_des}${str_site}${str_twitter}`
}
...
function album_des(pinfo){
	var des = ""
	if(pinfo){
		if(pinfo.album){
			des = `『${pinfo.csinger}』 Album 「${pinfo.album}」 \n` 
		}
	}
	return des
}
